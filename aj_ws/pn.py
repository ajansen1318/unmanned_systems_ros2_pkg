...
def main() -> None:
    ...
    time_now = get_time_in_secs(turtlebot_pursuer)

    # since our lidar is super slow we're going to set this node to match our
    # lidar rate to about 3 times its sampling rate
    thread = threading.Thread(target=rclpy.spin, args=(turtlebot_pursuer,), daemon=True)
    thread.start()
    ...
    while rclpy.ok():
        ...
        time_diff = get_time_in_secs(turtlebot_pursuer) - time_now
        rate.sleep()
        ...
        pursuer_position = np.array(turtlebot_pursuer.current_position)
        ...
        if check_close(turtlebot_pursuer.current_position, evader_position) == True:
