...
def main() -> None:
    ...
    # to find and avoid (hopefully) the pursuer
    lidar_freq = 5.0  # Hz
    thread = threading.Thread(target=rclpy.spin, args=(turtlebot_evader,), daemon=True)
    thread.start()
    rate = turtlebot_evader.create_rate(lidar_freq * 3)
    ...
    while rclpy.ok():
        ...
        desired_heading, heading_error, dist_error = gimme_da_loot(
            turtlebot_evader, goal_waypoint
        )
