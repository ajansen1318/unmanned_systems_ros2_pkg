...
def main() -> None:
    ...
    evader_start_position = (2, 2)
    evader_goal_position = (9, 9)

    if set_random == False:
        waypoints = [evader_goal_position]
    else:
    ...
    while rclpy.ok():
        ...
        while True:
            desired_heading, heading_error, dist_error = gimme_da_loot(
                turtlebot_evader, waypoint
            )

            print(
                "current heading is", m.degrees(turtlebot_evader.orientation_euler[2])
            )

            if abs(dist_error) < dist_tolerance and abs(heading_error) < heading_tol:
                turtlebot_evader.move_turtle(stop_speed, stop_speed)
                break  # exit the inner loop when conditions are met
    ...
    #     while (abs(dist_error) >= dist_tolerance) or (
    #         abs(heading_error) >= heading_tol
    #     ):
    #         print(
    #             "current heading is",
    #             m.degrees(turtlebot_evader.orientation_euler[2]),
    #         )
    #         # print("desired heading is", m.degrees(desired_heading), heading_error)

    #         if (
    #             abs(dist_error) >= dist_tolerance
