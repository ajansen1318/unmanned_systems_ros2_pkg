...
class Agent:
    ...
    def move(self, ang_vel: float, lin_vel: float, dt: float) -> None:
        """
        ...
        new_x = (lin_vel * np.cos(self.heading) * dt) + self.position[0]
        new_y = (lin_vel * np.sin(self.heading) * dt) + self.position[1]
...
if __name__ == "__main__":
    ...
    for i in range(N):
        # compute current los
        desired_heading_rad = compute_heading(pursuer.position, evader.position)

        current_distance = compute_distance(pursuer.position, evader.position)

        # figure out where this would have to go and how it actually moves the turtlebot

        # # not really sure what's going on here tbh
        # x = current_distance * m.cos(desired_heading_rad)
        # y = current_distance * m.sin(desired_heading_rad)

        # evader_x = pursuer.x + x
        # evader_y = pursuer.y + y
        # # idk i think it has to move
        # new_x, new_y = evader.move()
        # evader_vel_x = (new_x - evader_x) / dt
        # evader_vel_y = (new_y - evader_y) / dt
        ...
        if guidance_option == pro_nav_options[0]:
            flight_path_rate = pro_nav.simple_pro_nav(desired_heading_rad, dt)
        ...
        if guidance_option == pro_nav_options[1]:
            ...
            flight_path_rate, vel_cmd = pro_nav.true_pro_nav(
                pursuer.position, evader.position, dt, target_vel, pursuer_vel
            )
