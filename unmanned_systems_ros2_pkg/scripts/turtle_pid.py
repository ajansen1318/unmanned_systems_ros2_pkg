...
def main() -> None:
    ...
    des_heading_angle = np.deg2rad(225)  # desired yaw angle
    ...
    try:
        while rclpy.ok():
            # current_error = pid_angular.compute_error(des_heading_angle, turtlebot_node.orientation_euler[2])
            # print("my heading error", np.deg2rad(current_error))
            angular_gains = pid_angular.get_gains(
                des_heading_angle, turtlebot_node.orientation_euler[2]
            )
